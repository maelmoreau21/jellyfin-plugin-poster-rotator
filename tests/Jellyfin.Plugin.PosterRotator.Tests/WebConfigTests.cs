...
public sealed class WebConfigTests
{
    ...
    public void ConfigPage_RendersPoolImagePreviewsWithFallback()
    {
        ...
        Assert.Contains("grid-template-columns: repeat(auto-fill, minmax(148px, 160px));", html);
        Assert.Contains("width: 160px;", html);
        Assert.Contains("height: 240px;", html);
        Assert.Contains("max-height: 240px;", html);
