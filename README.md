...
- Plugin version: `1.8.4.4`
- Target ABI: Jellyfin `12.0.0.0`
- Jellyfin packages: `12.0.0` (stable)
- Runtime: `.NET 10`

Version `1.8.4.4` does not make raw SQL access. It uses Jellyfin services (`ILibraryManager`, `IProviderManager`) and stores its state in the plugin's data folder.
...
For a manual zip install, the `Jellyfin.Plugin.PosterRotator-1.8.4.4.zip` archive also contains `meta.json` and `jellyfin-plugin-posterrotator.png`, so that Jellyfin's plugin page can display the plugin image.
