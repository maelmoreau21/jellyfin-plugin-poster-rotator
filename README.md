...
For a manual zip install, the `Jellyfin.Plugin.PosterRotator-1.8.5.3.zip` archive also contains `meta.json` and `jellyfin-plugin-posterrotator.png`, so that Jellyfin's plugin page can display the plugin image.
...
1. Follow the build/package steps in [instructions.md](./instructions.md) to generate the release zip (e.g., `Jellyfin.Plugin.PosterRotator-1.8.5.3.zip`).
2. Calculate its MD5 checksum and update `manifest.json`.
3. Create the matching release tag (e.g., `v1.8.5.3`) on GitHub and attach `Jellyfin.Plugin.PosterRotator-1.8.5.3.zip`.
4. Ensure the download URL defined in `manifest.json` (`https://github.com/maelmoreau21/jellyfin-plugin-poster-rotator/releases/download/v1.8.5.3/Jellyfin.Plugin.PosterRotator-1.8.5.3.zip`) matches.
